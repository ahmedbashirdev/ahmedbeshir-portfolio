...
- **D9 — The digital/QR key is a lock-vendor integration, not built in-house.** A QR code alone cannot open a physical door — the unlock mechanism depends on the hotel's door hardware (Bluetooth/BLE, NFC, PIN-code, or a QR-reader lock). Even Duve — a mature competitor — does not build locks; it integrates with smart-lock vendors (ASSA ABLOY, Dormakaba, Salto, Nuki, etc.). Escalé will do the same: integrate with **one** lock vendor chosen per the hotel's hardware, and the lock hardware + the vendor subscription are the **client's cost** (excluded, like hosting and gateway fees). The in-app QR stays as the UI / check-in pass; the real unlock = whatever the integrated lock supports. **Open:** choose the lock vendor and confirm with the client before development; the JWT-QR "offline-validating" model in `PROJECT_CONTEXT.md` §6 should be revisited — it assumes QR-reader locks, which is a narrow hardware choice.
- **D10 — Target market is Saudi Arabia (launch market), not Egypt.** The client is based in Saudi and will market the product there first; the platform stays multi-tenant and **multi-market** (Egypt and the wider region are possible later). Earlier docs said "Egyptian market" — corrected 2026-05-25. Applied: payments → Saudi rails (mada, Apple Pay, STC Pay, Tabby/Tamara — not Fawry); identity/compliance → Nafath, PDPL (SDAIA), ZATCA/Fatoora e-invoicing, MoI guest registration (Shomoos); screen example data localised to Saudi (Riyadh, Jeddah, AlUla, Diriyah; Adrère Amellal → Dar AlUla; SAR currency); brand framing shifts from "North African Soul" toward an Arabian/Najdi feel. `PROJECT_CONTEXT.md` and `CLAUDE.md` updated. **Closed 2026-05-25:** `design-system.md` brand references reworked for Saudi (Najdi/Diriyah, AlUla, the Red Sea; "an Arabian Soul"; SAR currency examples); and the proposal's payment line updated to mada/Apple Pay with the PDF regenerated.
